...
Sweeps are saved in a csv format that mimics those saved locally on the N9040B, containing lines of parameters with their values separated by commas as shown in Figure~\ref{fig:csvexample}. These sweeps can be converted to the DRIFT format by using the \lstinline|DRIFT Processing| utlity, accessed from the ribbon on the top left of the interface. The user can convert a directory of traces from the local format to the DRIFT format immediately or schedule it to be done daily at a certain time. Once DRIFT is running in a closed-access build, this document will be updated with the process for uploading files to DRIFT.\@
