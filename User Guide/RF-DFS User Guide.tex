...
\title{New Mexico Spectrum Monitor Operator's Guide, Build 1.0.0}
\author{R. Nguyen
    }% <-this % stops a space
\date{Jan 1, 2000}
\def\docnum{DOC NUM}
...
\subsection{What is the New Mexico Spectrum Monitor?}
The NMSM is a tool utilized by the NRAO-NM Interference Protection Group to identify and locate RF interference at the Very Large Array. It combines two previously separate systems: the Radio Frequency - Direction Finding System (RF-DFS) and Environmental Monitoring System (RF-EMS) into one combined spectrum monitor with omnidirectional and directional capabilities. The former is a 3-meter dish antenna with azimuth and elevation drives whereas the latter is a 50-foot tower with a nominally 0 dBi club antenna. Both are connected to the RFI shack and are shown in Fig~\ref{fig:livepic}. A Python front end interface allows the user to control RF switching between both antennas and log data through a spectrum analyzer. 
\begin{figure}[!htb]
  \begin{center}
...
The build associated with this document features:
\begin{enumerate}
  \item Continuous 50 - 10000 MHz monitoring with integrated access to spectrum analyzer commands.
  \item 0 -- 360 degree azimuth and 0 -- 90 degree elevation directional monitoring.
  \item Combined functionality of the RF-DFS and RF-EMS that allows for remote operation.
  \item Python scripting and automation to queue operations at any time.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item One Mean Well LRS-75-24 24V 3.2A switching power supply
  \item One Mean Well LRS-75-5 12V 6A switching power supply
\end{enumerate}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \matrix[wire board, wiresright] (plc) {
        ...
        % plc to shack switch
        \draw(plc-10-3.south west) -- (plc-10-3.south west -| m) node[circ](v2circ){} -- (shackswitch-1-1.south east -| m) node[circ](v2circ2){} -- (shackswitch-1-1.south east);
        ...
        \draw(plc-1-3.south west) -- (plc-1-3.south west -| m) -- (v2circ);
        \draw(v2circ2)[->] -- (v2circ2 |- plc-17-3.south west) -- (plc-17-3.south west -| 5e) coordinate(topower+) node[anchor=west]{\footnotesize To LRS-75-24 +24V};
        % plc ground wires
        \draw(plc-18-3.south west) -- (plc-18-3.south west -| 3e) node[circ](gndnode3){} -- (shackswitch-2-1.south east -| 3e) node[circ=1pt](gndnode1){};
        ...
        \draw(gndnode3)[->] -- (gndnode3 -| topower+) node[anchor=west]{\footnotesize To Common GND};
        % lps cont
        ...
        \draw(ellipse)+(0, 2.5) node[anchor=south, align=center]{\footnotesize CAT6 Cable (x2)\\\footnotesize See Fig.~\ref{fig:dfscat5wiring}, \ref{fig:emscat5wiring}\\\footnotesize DFS, EMS --->};
        % plc to loto
        \draw(v2circ -| 5e) node[anchor=west, name=12vlabel]{\footnotesize To LRS-75-12 +12V};
        \draw(12vlabel.east)[<-] -- (v2circ -| lps.east) coordinate(lotostart);
        \draw(lotostart)[->] to[switch, name=loto] ++ (2, 0) -- (v2circ -| p3);
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \matrix[wire board, wiresright] (converter) {
        ...
        \draw(converter-1-3.south west) -- (lps-1-1.south east);
        \draw(converter-2-3.south west) -- (lps-2-1.south east);
        \draw(converter-3-3.south west) -- (lps-3-1.south east);
        \draw(converter-4-3.south west) -- (lps-4-1.south east);
        \draw(converter-5-3.south west) -- (converter-5-2 -| 3eighth) -- (lps-6-2 -| 5eighth) -- (lps-6-1.south east);
        \draw(lps-5-1.south east)[->] -- (lps-5-2 -| m) -- ++ (0, -1) node[anchor=north]{\scriptsize To LOTO (Fig.~\ref{fig:shackwiring})};
        ...
        \draw(ellipse |- lps2-1-3)+(0, 0.5) node[anchor=south, align=center]{\footnotesize CAT6 Cable\\\footnotesize DFS --->\\\footnotesize <-- Shack (Above)};
...
\begin{figure}[!htb]
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw(ellipse)+(0, 2.5) node[anchor=south, align=center]{\footnotesize CAT6 Cable\\\footnotesize DFS --->\\\footnotesize <-- Shack (Fig.~\ref{fig:shackwiring})};
...
The feed and LNA housing are mounted at the apex of the dish with four all-thread rods in a square pattern.
...
\begin{figure}[!htb]
  \begin{center}
    \begin{tikzpicture}
      ...
      \path(lps-4-3) -- (lps-5-3) coordinate[pos=0.5](lps2midway);
      \draw(lps2midway)+(-4, 0) ellipse(0.5 and 2.5) coordinate(ellipse);
      \draw(ellipse)+(0, 2.5) node[anchor=south, align=center]{\footnotesize CAT6 Cable\\\footnotesize EMS -->\\\footnotesize <-- Shack (Fig.~\ref{fig:shackwiring})};
\end{tikzpicture}
\end{center}
...
\subsection{Safety}
When servicing any of the hardware or allowing other parties to service hardware, the operator is responsible from preventing hazardous conditions which may result in injury. DFS motor operation should be disabled with the lockout-tagout switch to prevent a remote operator or automated script from moving the dish while individuals are within the fenced area. Additionally, there is an AC breaker behind the RFI shack that can be used to kill power to the tower, dish, or shack as needed. This breaker also powers the VLA weather station so the operator should only disconnected the necessary breakers for servicing. Lastly, when operating the DFS remotely, the strobe light should be enabled 30 seconds prior to dish movement to warn on-site individuals.
...
In order to use the front-end interface, download the following deployment requirements and the latest release on \verb|EMSMONPC2|, then run the executable. Releases are located at the following link: \url{https://github.com/RomiFC/nm-spectrum-monitor/releases}.
\begin{enumerate}
  \item NI-VISA
...
From here, the user can issue SCPI commands to the spectrum analyzer using the control panel on the right, issue motor commands using the \verb|Antenna Position| panel, or select RF chains using the \verb|PLC Operations| panel on the left after it has been initialized.

Additionally, saved traces can be opened as an interactable matplotlib figure by selecting \verb|File>Open trace| in the top left banner. 
...
The automation configuration window can be opened by clicking the \verb|Queue| button on the left hand side control panel. This allows the user to queue Python operations on a time basis from 1 minute to 24 hours via the config tab shown in Figure~\ref{fig:automationconfig}. The backend for this utility is the \verb|APScheduler| module which generates a \verb|BackgroundScheduler| thread to handle job stores, executors, and triggers, when the program is initialized. The \verb|Start/Stop| button on the left hand panel of the root window in Figure~\ref{fig:gui} will generate a thread with the target function \verb|initSchedule|, add a job to the scheduler for each queued time slot generated (Shown in Figure~\ref{fig:automationconfig}), and resume scheduler job processing. The target function of scheduled jobs is \verb|onSchedule| which can be configured in the scripting tab shown in Figure~\ref{fig:automationscripting} along with \verb|initSchedule|.

\begin{figure}[!htb]
  \begin{center}
    \includegraphics[width=\textwidth]{images/7autoconfig.png}
  \end{center}
  \caption{Automation Panel Configuration Tab.}\label{fig:automationconfig}
\end{figure}

\begin{figure}[!htb]
  \begin{center}
    \includegraphics[width=\textwidth]{images/8autoscript.png}
  \end{center}
  \caption{Automation Panel Scripting Tab.}\label{fig:automationscripting}
\end{figure}

The scripting tab also contains some example preset functions that collect different trace types for reference. As a precaution, the scripted function should maintain any acquired thread locks for as long as necessary to prevent reacquiring the same lock during execution. If not, it's possible that a loop thread could acquire the lock, resulting in race conditions if it modifies the resource in a manner that the job thread is not expecting. The job scheduler also doesn't prevent the user from performing any actions while a job is active, meaning spectrum analyzer, PLC, and motor operations, can still be performed. If the user attempts to access a locked resource while a job is active, it is likely that the program or operation will become unresponsive until the job thread releases that lock.

By default, the \verb|thread_max_workers| value is set to 1, meaning the scheduler can only generate one thread at a time and by extension, only one job can be active at a time. If a job is still active while a queued job is meant to trigger, it will be skipped and processed once the active job concludes operation. At this moment, the scheduler will either execute the skipped job if the \verb|misfire_grace_time| is greater than the elapsed time, or remove the job depending on the \verb|coalesce| setting. More information on \verb|misfire_grace_time| and \verb|coalesce| is available in the \verb|APScheduler| documentation.

\subsubsection{Waterfall Plots}
One of the primary functions of the NMSM is to allow for 24/7 spectrum monitoring. The principle means by which this is achieved is by collecting sweeps at an interval using the automation script and generating a waterfall plot with the tool under \verb|Run>Waterfall Plot Utility|. From here, the user can configure a scheduler to generate a 24 hour waterfall plot using all traces within the \verb|Trace Directory| that have a matching date and receiver. If the amount of traces is above the \verb|Threshold|, the utility will generate a matplotlib waterfall plot in the format under \verb|File Type|, a plotly.js waterfall plot in \verb|html| format, and an \textit{averaged} trace over the day.
...
The following subsections (and development of the user interface) require extensive knowledge of Python, tkinter, matplotlib, multithreading, serial and VISA communications, and SCPI commands. Additionally, the documentation for all of the aforementioned modules should be regularly referenced, including the N9040B \textit{SA Mode User's and Programmer's Reference Manual}.
...
\begin{table}[!htb]\rowcolors{1}{lightgray!30}{white}
  \begin{center}
...
To add a specific parameter, see the N9040B \textit{SA Mode User's and Programmer's Reference} for the associated SCPI command. In this case, it is \verb|[:SENSe]:SWEep:POINts|, as shown in Figure~\ref{fig:paraminstantiation}. Create an object with a string for the name of the parameter and the SCPI command associated with it. The attribute \verb|log| is a boolean variable which determines if the parameter should be saved in trace CSVs.
...
\begin{figure}[!htb]
  \begin{center}
    \begin{circuitikz}
      ...
      \draw(sw4.out 2) to[amp, invert, t=\ctikzflipx{\small GB}] (sw5.out 2);
      \draw(sw5.in) -- ++ (1, 0)
      node[rotary switch = 4 in 60 wiper 60, anchor=in](sw1){};
    ...
    \end{circuitikz}
  \end{center}
  \caption{Optional Gain Block Amplifier Inside Shack.}\label{fig:poweramplifier}
\end{figure}
\begin{table}[!htb]\rowcolors{1}{lightgray!30}{white}
  \begin{center}
...
The Dynamic Radio Interference Finding Tool (DRIFT) is an Electromagnetic Spectrum Management project intended to be an external-facing, web-based interface to visualize RFI data from various sites including VLA, GBT, VLBA, CHIME, ASM-2, and the NMSM.\@

Sweeps are saved in a csv format that mimics those saved locally on the N9040B, containing lines of parameters with their values separated by commas as shown in Figure~\ref{fig:csvexample}. These sweeps can be converted to the DRIFT format by using the \verb|DRIFT Processing| utlity, accessible from \verb|Run>DRIFT Processing| in the top left banner. The user can convert a directory of traces from the local format to the DRIFT format immediately or schedule it to be done daily at a certain time. Once DRIFT is running in a closed-access build, this document will be updated with the process for uploading files to DRIFT.\@
...
\begin{table}[!htb]\rowcolors{1}{lightgray!30}{white}
  \begin{center}
    \begin{tabularx}{\linewidth}{l|l|X}
      ...
      \lstinline|spw_id|        & No  & Spectral window - only useful for detecting baseband overlaps in instruments like the VLA or GBT.\\
...
NMSM,EMS1,LV,dBm,EMS1-2025-03-28-AVG-D,2025-03-28T11:49:55.199981+00:00,10000000,-71.1912847253302
,,,,,,21990000,-105.183861324451
,,,,,,33980000,-72.2399984094122
\end{lstlisting}
\begin{lstlisting}[aboveskip=0pt, belowskip=0pt, numbers=none]
